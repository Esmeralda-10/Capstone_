...
// These endpoints can be accessed without authentication for public display

// Database connection for API endpoints
try {
    $pdo_api = new PDO("mysql:host=localhost;dbname=pest control;charset=utf8mb4", "root", "", [
        PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION,
        PDO::ATTR_DEFAULT_FETCH_MODE => PDO::FETCH_ASSOC
    ]);
} catch (PDOException $e) {
    // If database connection fails for API, return error
...
/* ==================== GET SERVICES FOR INDEX.HTML ==================== */
if (isset($_GET['action']) && $_GET['action'] === 'get_services') {
    header('Content-Type: application/json');
    
    try {
        // Try to get all columns from services table
        $stmt = $pdo_api->query("SELECT * FROM services ORDER BY service_name");
        ...
        foreach ($services as $service) {
            $formatted_services[] = [
                'id' => isset($service['service_id']) ? (int)$service['service_id'] : 0,
                'title' => isset($service['service_name']) ? $service['service_name'] : 'Unnamed Service',
                'type' => isset($service['service_type']) ? $service['service_type'] : '',
                'description' => isset($service['service_details']) ? $service['service_details'] : (isset($service['details']) ? $service['details'] : ''),
        ...
        echo json_encode([
            ...
            'debug' => count($formatted_services) > 0 ? 'Services found' : 'No services in database'
        ]);
    } catch (PDOException $e) {
...
// Only require authentication for admin pages, not for public API endpoints
if (!isset($_GET['action']) || $_GET['action'] !== 'get_services') {
...
function logAudit($pdo, $action, $table_name = null, $record_id = null, $old_values = null, $new_values = null) {
    try {
        ...
        $stmt = $pdo->prepare("INSERT INTO audit_logs (user_id, username, action, table_name, record_id, old_values, new_values, ip_address, user_agent) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([$user_id, $username, $action, $table_name, $record_id, $old_values, $new_values, $ip_address, $user_agent]);
        return true;
    } catch (PDOException $e) {
    ...
    try {
        if (isset($_POST['add_booking'])) {
            ...
            $new_values = [
                ...
                'structure_types' => $_POST['structure_type'],
                'price_range' => $_POST['price_range'],
                'appointment_date' => $_POST['appointment_date'],
                'appointment_time' => $_POST['appointment_time'],
                'status' => 'Pending'
            ];
            logAudit($pdo, 'Create Record', 'service_bookings', $booking_id, null, $new_values);
        ...
        if (isset($_POST['delete_booking'])) {
            ...
            $pdo->prepare("DELETE FROM service_bookings WHERE booking_id = ?")->execute([$booking_id]);
            
            // Log audit
            if ($booking) {
                logAudit($pdo, 'Delete Record', 'service_bookings', $booking_id, $booking, null);
            }
        ...
        if (isset($_POST['update_status'])) {
            ...
            if (empty($new_status) || !in_array($new_status, $allowed_statuses)) {
                ...
                if (!$booking) {
                    ...
                    $new_values = [
                        'status' => $new_status,
                        'admin_message' => !empty($admin_message) ? substr($admin_message, 0, 200) : null
                    ];
                    logAudit($pdo, 'Update Record', 'service_bookings', $booking_id, $old_values, $new_values);
                    ...
                    if ($booking && !empty($booking['email'])) {
                        ...
                        // Log email audit (always log, even if failed)
                        $emailData = [
                            'recipient' => $to,
                            'subject' => $subject,
                            'type' => 'Status Update',
                        ...
                        logAudit($pdo, 'Send Email', null, null, null, $emailData);
                    }
        ...
        if (isset($_POST['reschedule_booking'])) {
            ...
            $reschedule_message = trim($_POST['reschedule_message'] ?? '');
            ...
            if (empty($new_date) && empty($new_time)) {
                ...
                if (!$booking) {
                    ...
                    if (!empty($updateFields)) {
                        ...
                        // Log audit for reschedule
                        $new_values = [];
                        if (!empty($new_date)) $new_values['appointment_date'] = $new_date;
                        if (!empty($new_time)) $new_values['appointment_time'] = $new_time;
                        if (!empty($reschedule_message)) $new_values['reschedule_message'] = substr($reschedule_message, 0, 200);
                        logAudit($pdo, 'Update Record', 'service_bookings', $booking_id, $old_values, $new_values);
                        ...
                        if ($updatedBooking && !empty($updatedBooking['email'])) {
                            ...
                            $defaultMessage = "We have rescheduled your appointment. Please see the new appointment details below.";
                            
                            // Use custom message if provided, otherwise use default
                            $messageContent = !empty($reschedule_message) ? nl2br(h($reschedule_message)) : $defaultMessage;
                            ...
                            <head>
                                ...
                                <style>
                                    ...
                                    .message-box { background: #e0f2fe; padding: 15px; margin: 15px 0; border-radius: 8px; border-left: 4px solid #0ea5e9; }
                                    .footer { text-align: center; margin-top: 30px; color: #64748b; font-size: 12px; }
                                </style>
                            </head>
                            <body>
                            ...
                            // Add message box if message exists
                            ...
                            // Log email to audit logs (always log, even if failed)
                            $emailData = [
                                'recipient' => $to,
                                'subject' => $subject,
                                'type' => 'Reschedule',
        ...
        if (isset($_POST['upload_pictures'])) {
            ...
            if (isset($_FILES['pictures']) && !empty($_FILES['pictures']['name'][0])) {
                ...
                if (!empty($uploaded_files)) {
                    // Log audit for picture upload
                    logAudit($pdo, 'Update Record', 'service_bookings', $booking_id, null, [
                    ...
                    if ($booking && !empty($booking['email'])) {
                        ...
                        $emailData = [
                            'recipient' => $to,
                            'subject' => $subject,
                            'type' => 'Pictures Uploaded',
                            'booking_id' => $booking_id,
                            'reference_code' => $booking['reference_code'] ?? 'N/A',
                            'customer_name' => $booking['customer_name'] ?? 'N/A',
                            'pictures_count' => count($uploaded_files),
                            'status' => $emailSent ? 'Sent' : 'Failed'
                        ];
                        logAudit($pdo, 'Send Email', null, null, null, $emailData);
                    }
        ...
        if (isset($_POST['add_announcement'])) {
            ...
            logAudit($pdo, 'Create Record', 'announcements', $announcement_id, null, [
                'title' => $_POST['announcement_title'],
                'announcement_date' => $_POST['announcement_date'],
                'announcement_time' => $_POST['announcement_time'] ?? null,
                'color' => $_POST['announcement_color'] ?? '#ff6b6b'
            ]);
        ...
        if (isset($_POST['edit_announcement'])) {
            ...
            $pdo->prepare("UPDATE announcements SET title=?, description=?, announcement_date=?, announcement_time=?, color=? WHERE announcement_id=?")
                ->execute([$_POST['announcement_title'], $_POST['announcement_description'] ?? '', $_POST['announcement_date'], $_POST['announcement_time'] ?? null, $_POST['announcement_color'] ?? '#ff6b6b', $announcement_id]);
            
            logAudit($pdo, 'Update Record', 'announcements', $announcement_id, $old_announcement, [
                'title' => $_POST['announcement_title'],
        ...
        if (isset($_POST['delete_announcement'])) {
            ...
            $pdo->prepare("DELETE FROM announcements WHERE announcement_id = ?")->execute([$announcement_id]);
            
            if ($old_announcement) {
                logAudit($pdo, 'Delete Record', 'announcements', $announcement_id, $old_announcement, null);
            }
        ...
        if (isset($_POST['add_service'])) {
            ...
            if ($existing) {
                ...
                try {
                    ...
                    try {
                        $pdo->exec("ALTER TABLE services ADD COLUMN category VARCHAR(100) DEFAULT NULL");
                    } catch (PDOException $e) {
                        // Column might already exist, ignore
                    }
                    
                    $pdo->prepare("INSERT INTO services (service_name,service_type,service_details,active_ingredient,category) VALUES (?,?,?,?,?)")
                        ->execute([$_POST['service_name'], $_POST['service_type'], $_POST['service_details'] ?? '', $active_ingredient, $category]);
                    
                    $service_id = $pdo->lastInsertId();
                    logAudit($pdo, 'Create Record', 'services', $service_id, null, [
        ...
        if (isset($_POST['edit_service'])) {
            ...
            $pdo->prepare("UPDATE services SET service_name=?,service_type=?,service_details=?,active_ingredient=?,category=? WHERE service_id=?")
                ->execute([$_POST['service_name'], $_POST['service_type'], $_POST['service_details'] ?? '', $active_ingredient, $category, $service_id]);
            
                    logAudit($pdo, 'Update Record', 'services', $service_id, $old_service, [
                        'service_name' => $_POST['service_name'],
                        'service_type' => $_POST['service_type'],
                        'service_details' => $_POST['service_details'] ?? '',
                        'active_ingredient' => $active_ingredient,
        ...
        if (isset($_POST['delete_service'])) {
            ...
            if ($old_service) {
                logAudit($pdo, 'Delete Record', 'services', $id, $old_service, null);
            }
        ...
        if (isset($_POST['add_price'])) {
            ...
            $price_range_id = $pdo->lastInsertId();
            logAudit($pdo, 'Create Record', 'service_price_ranges', $price_range_id, null, [
                'service_id' => $_POST['service_id'],
                'price_range' => $_POST['price_range'],
                'price' => $_POST['price']
            ]);
        ...
        if (isset($_POST['edit_price'])) {
            ...
            // Get old values
            $stmt = $pdo->prepare("SELECT * FROM service_price_ranges WHERE price_range_id = ?");
            ...
            $pdo->prepare("UPDATE service_price_ranges SET price_range=?,price=? WHERE price_range_id=?")
                ->execute([$_POST['price_range'], $_POST['price'], $price_range_id]);
            
            logAudit($pdo, 'Update Record', 'service_price_ranges', $price_range_id, $old_price, [
                'price_range' => $_POST['price_range'],
                'price' => $_POST['price']
            ]);
        ...
        if (isset($_POST['delete_price'])) {
            ...
            $pdo->prepare("DELETE FROM service_price_ranges WHERE price_range_id = ?")->execute([$price_range_id]);
            
            if ($old_price) {
                logAudit($pdo, 'Delete Record', 'service_price_ranges', $price_range_id, $old_price, null);
            }
        ...
        // Handle Gmail compose logging
        if (isset($_POST['log_gmail_compose'])) {
            $emailData = [
                'recipient' => $_POST['recipient'] ?? '',
                'subject' => $_POST['subject'] ?? '',
                'type' => $_POST['email_type'] ?? 'Gmail Compose',
                'booking_id' => $_POST['booking_id'] ?? null,
                'reference_code' => $_POST['reference_code'] ?? 'N/A',
            ...
            header('Content-Type: application/json');
            echo json_encode(['success' => true, 'message' => 'Gmail compose logged']);
            exit();
        }
    } catch (Exception $e) {
...
<head>
    ...
    <title>Techno Pest Control • Manager Dashboard</title>
        ...
        <div class="tab-content" id="services">
            <div class="content-card">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    ...
                    <button class="btn btn-modern btn-success-modern" data-bs-toggle="modal" data-bs-target="#addServiceModal">
                    ...
                    <div class="col-lg-6">
                        <div class="card border-0 shadow-lg h-100" style="border-left: 5px solid var(--primary) !important; border-radius: 16px;">
                            <div class="card-body">
                                ...
                                <div class="mb-2">
                                    <span class="badge" style="background: rgba(16, 185, 129, 0.1); color: var(--primary); font-size: 0.75rem; padding: 0.3rem 0.6rem;">
                                        <i class="bi bi-tag me-1"></i><?= h($s['category']) ?>
                                    </span>
                                </div>
                                <?php endif; ?>
                                
                                <div class="mb-3">
                                    <?php if (!empty($s['service_details'])): ?>
                                    <p class="text-muted mb-0" style="line-height: 1.6;"><?= nl2br(h($s['service_details'])) ?></p>
                                    <?php else: ?>
                                    <p class="text-muted mb-0" style="font-style: italic;">No description provided</p>
                                    <?php endif; ?>
                                </div>
                                
                                <?php if (!empty($s['active_ingredient'])): ?>
                                <div class="mb-3">
                                    <small class="text-muted d-block mb-1" style="font-weight: 600;">Active Ingredient:</small>
                                    <span class="badge" style="background: rgba(99, 102, 241, 0.1); color: #6366f1; font-size: 0.8rem; padding: 0.3rem 0.6rem;">
                                        <?= h($s['active_ingredient']) ?>
                                    </span>
                                </div>
                                <?php endif; ?>
                                
                                <div class="d-flex gap-2 mb-3">
                                    <button class="btn btn-sm btn-warning-modern" onclick="editService(<?= htmlspecialchars(json_encode($s)) ?>)" title="Edit service">
                                        <i class="bi bi-pencil me-1"></i>Edit
                                    </button>
                                    <form method="post" class="d-inline" onsubmit="return confirm('Delete this service? This will also delete all associated prices.')">
                                        <input type="hidden" name="service_id" value="<?= $s['service_id'] ?>">
                                        <button type="submit" name="delete_service" class="btn btn-sm btn-danger-modern" title="Delete service">
                                            <i class="bi bi-trash me-1"></i>Delete
                                        </button>
                                    </form>
                                </div>
                                <hr>
                                <h6 class="fw-bold mb-3"><i class="bi bi-currency-exchange me-2"></i>Pricing:</h6>
                                <div class="mb-3">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <input type="text" name="service_name" class="form-control" placeholder="e.g., Termite Control" required>
                    </div>
                    <div class="row g-3 mb-3">
                        <div class="col-md-6">
                            <label class="form-label fw-bold">Service Type <span class="text-danger">*</span></label>
                            <input type="text" name="service_type" class="form-control" placeholder="e.g., Treatment" required>
                        </div>
                        <div class="col-md-6">
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-bold">Active Ingredient</label>
                        <input type="text" name="active_ingredient" class="form-control" placeholder="e.g., Fipronil, Lambda-Cyhalothrin">
                        <small class="text-muted">Optional: Active ingredient used for this service</small>
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-bold">Service Details</label>
                        <textarea name="service_details" class="form-control" rows="5" placeholder="Describe the service, treatment methods, benefits, etc. This will be displayed on the homepage."></textarea>
                        <small class="text-muted">This description will appear on the homepage (index.html)</small>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-modern" style="background: var(--border); color: var(--dark);" data-bs-dismiss="modal">Cancel</button>
                    <button name="add_service" class="btn btn-modern btn-success-modern">
...
<div class="modal fade" id="editServiceModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <form method="post">
            <div class="modal-content">
                <div class="modal-header text-white" style="background: linear-gradient(135deg, var(--warning), #d97706);">
                    <h5><i class="bi bi-pencil me-2"></i>Edit Service</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    ...
                    <div class="row g-3 mb-3">
                        ...
                        <div class="col-md-6">
                            <label class="form-label fw-bold">Category</label>
                            <select name="category" id="edit_service_category" class="form-select">
                                <option value="">Select Category</option>
                                <option value="Termite Control">Termite Control</option>
                                <option value="General Pest Control">General Pest Control</option>
                                <option value="Rodent Control">Rodent Control</option>
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-bold">Active Ingredient</label>
                        <input type="text" name="active_ingredient" id="edit_service_active_ingredient" class="form-control" placeholder="e.g., Fipronil, Lambda-Cyhalothrin">
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-bold">Service Details</label>
                        <textarea name="service_details" id="edit_service_details" class="form-control" rows="5" placeholder="Describe the service, treatment methods, benefits, etc."></textarea>
                        <small class="text-muted">This description will appear on the homepage (index.html)</small>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-modern" style="background: var(--border); color: var(--dark);" data-bs-dismiss="modal">Cancel</button>
                    <button name="edit_service" class="btn btn-modern btn-warning-modern">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="mt-3">
                            ...
                            <textarea name="reschedule_message" id="reschedule_message" class="form-control" rows="4" placeholder="Add a custom message to include in the reschedule email notification. If left blank, a default message will be sent."></textarea>
                            <small class="text-muted">This message will be included in the reschedule email sent to the customer.</small>
                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const rescheduleMessageField = document.getElementById('reschedule_message');
    const rescheduleDateField = document.getElementById('status_new_appointment_date');
    const rescheduleTimeField = document.getElementById('status_new_appointment_time');
    const adminMessageField = document.getElementById('status_admin_message');
    ...
    // Function to generate status message with date and time
    function generateStatusMessage(status, date, time) {
        let message = '';
        
        if (status === 'Pending') {
            message = 'Your booking is currently pending and will be reviewed by our team shortly.';
        } else if (status === 'In Progress') {
            message = 'Your booking is now in progress. Our team is working on your service request.';
        } else if (status === 'Completed') {
            message = 'Your booking has been completed. Thank you for choosing our services!';
        ...
        if (date || time) {
            ...
            if (date) {
                message += 'Appointment Date: ' + formatDate(date);
            }
    ...
    // Default messages for status updates (without date/time - used as fallback)
    ...
    // Function to generate reschedule message with date and time
    function generateRescheduleMessage(date, time) {
        let message = "We have rescheduled your appointment";
        ...
        message += "Please see the new appointment details below. If you have any questions or concerns, please don't hesitate to contact us.";
        
        return message;
    }

    // Auto-populate status message when status is selected
    if (statusField && adminMessageField) {
        let statusMessageAutoFilled = false;
        let lastAutoFilledMessage = '';
        
        function getBookingData() {
            // Try to get booking data from global variable (defined later in script)
            if (typeof currentBookingData !== 'undefined' && currentBookingData.status) {
                return currentBookingData.status;
            }
            // Fallback: try to get from modal fields
            return {
                appointmentDate: document.getElementById('status_new_appointment_date')?.value || '',
                appointmentTime: document.getElementById('status_new_appointment_time')?.value || ''
            };
        }
        
        statusField.addEventListener('change', function() {
            const selectedStatus = this.value;
            if (selectedStatus && !statusMessageAutoFilled && !adminMessageField.value.trim()) {
                // Get appointment date and time from stored booking data
                const bookingData = getBookingData();
                const appointmentDate = bookingData.appointmentDate || '';
                const appointmentTime = bookingData.appointmentTime || '';
                
                // Generate message with date and time
                const generatedMessage = generateStatusMessage(selectedStatus, appointmentDate, appointmentTime);
                adminMessageField.value = generatedMessage;
                lastAutoFilledMessage = generatedMessage;
                statusMessageAutoFilled = true;
            } else if (selectedStatus && statusMessageAutoFilled) {
                // Update message if status changes and it was auto-filled
                const bookingData = getBookingData();
                const appointmentDate = bookingData.appointmentDate || '';
                const appointmentTime = bookingData.appointmentTime || '';
                
                // Check if current message matches the last auto-filled message
                if (adminMessageField.value.trim() === lastAutoFilledMessage) {
                    const generatedMessage = generateStatusMessage(selectedStatus, appointmentDate, appointmentTime);
                    adminMessageField.value = generatedMessage;
                    lastAutoFilledMessage = generatedMessage;
                }
        ...
        // Reset auto-fill flag when message is manually edited
        adminMessageField.addEventListener('input', function() {
            if (this.value.trim() !== lastAutoFilledMessage || this.value.trim() === '') {
                statusMessageAutoFilled = false;
                lastAutoFilledMessage = '';
            }
        });
        
        // Also auto-populate when modal is opened if status is already selected
        const statusUpdateModal = document.getElementById('statusUpdateModal');
        if (statusUpdateModal) {
            statusUpdateModal.addEventListener('shown.bs.modal', function() {
                // Small delay to ensure currentBookingData is set by openStatusModal
                setTimeout(function() {
                    const selectedStatus = statusField.value;
                    if (selectedStatus && !adminMessageField.value.trim()) {
                        const bookingData = getBookingData();
                        const appointmentDate = bookingData.appointmentDate || '';
                        const appointmentTime = bookingData.appointmentTime || '';
    ...
    // Auto-populate reschedule message when user starts interacting with reschedule fields
    if (rescheduleMessageField && rescheduleDateField && rescheduleTimeField) {
        let messageAutoFilled = false;
        
        // Function to update reschedule message with current date/time values
        function updateRescheduleMessage() {
            const date = rescheduleDateField.value;
            const time = rescheduleTimeField.value;
            
            if (!messageAutoFilled && !rescheduleMessageField.value.trim()) {
                rescheduleMessageField.value = generateRescheduleMessage(date, time);
                messageAutoFilled = true;
            } else if (messageAutoFilled) {
                // Update the message if it was auto-filled and date/time changes
                const currentMessage = rescheduleMessageField.value;
                // Check if message contains date/time pattern (to detect if it's auto-generated)
                if (currentMessage.includes('Date:') || currentMessage.includes('Time:')) {
                    rescheduleMessageField.value = generateRescheduleMessage(date, time);
                }
            }
        }
        
        // Auto-fill message when user enters date or time
        [rescheduleDateField, rescheduleTimeField].forEach(field => {
        ...
        // Reset auto-fill flag when message is manually cleared
        rescheduleMessageField.addEventListener('input', function() {
            if (this.value.trim() === '') {
                messageAutoFilled = false;
            } else {
                // Check if user manually edited the message (removed date/time pattern)
                if (!this.value.includes('Date:') && !this.value.includes('Time:')) {
                    messageAutoFilled = false;
                }
            }
        });
...
function editService(service) {
    ...
    document.getElementById('edit_service_details').value = service.service_details || '';
    document.getElementById('edit_service_active_ingredient').value = service.active_ingredient || '';
...
function openGmailCompose(event, modalType) {
    ...
    let subject = '';
    let body = '';
    let emailType = 'Gmail Compose';

    if (modalType === 'upload') {
        emailType = 'Pictures Uploaded';
        ...
        // Get form values
        const selectedStatus = document.getElementById('status_new_status')?.value || '';
        const newAppointmentDate = document.getElementById('status_new_appointment_date')?.value || '';
        const newAppointmentTime = document.getElementById('status_new_appointment_time')?.value || '';
        const rescheduleMessage = document.getElementById('reschedule_message')?.value || '';
        const adminMessage = document.getElementById('status_admin_message')?.value || '';
        
        // Check if status is selected (priority over reschedule)
        // If status is selected, always show status update (even if same as current)
        if (selectedStatus) {
            // Status update email
            ...
            // Check if adminMessage already contains status message or date/time info
            const statusMsg = statusMessages[selectedStatus] || '';
            const hasStatusMsg = statusMsg && adminMessage.includes(statusMsg);
            const hasDateInfo = adminMessage.includes('Appointment Date') || adminMessage.includes('Date:');
            const hasTimeInfo = adminMessage.includes('Appointment Time') || adminMessage.includes('Time:');
            const isCompleteMessage = hasStatusMsg || (hasDateInfo && hasTimeInfo);
            
            if (adminMessage.trim() && isCompleteMessage) {
                // Message already contains status info and/or date/time, use it as-is to avoid duplication
                body += `${adminMessage.trim()}\n`;
            } else if (adminMessage.trim()) {
                // Custom message provided but doesn't contain status/date info, add default status message first
                body += `${statusMsg || 'Your booking status has been updated.'}\n\n`;
                body += `${adminMessage.trim()}\n`;
                
                // Add appointment date and time if not already in custom message
                if ((appointmentDate || appointmentTime) && !hasDateInfo && !hasTimeInfo) {
                    body += `\n`;
                    if (appointmentDate) {
                        body += `Appointment Date: ${formatDateForGmail(appointmentDate)}\n`;
                    }
                    if (appointmentTime) {
                ...
                // No custom message, use default status message
                body += `${statusMsg || 'Your booking status has been updated.'}\n`;
                
                // Add appointment date and time if available
            ...
            // Reschedule email (only if no status change or status is same)
            emailType = 'Reschedule';
            // Reschedule email (only if no status change or status is same)
            const oldAppointmentDate = bookingData.appointmentDate || '';
            const oldAppointmentTime = bookingData.appointmentTime || '';
            
            subject = `Booking Rescheduled - ${referenceCode}`;
            body = `Dear ${customerName},\n\n` +
                   `Your booking appointment has been rescheduled.\n\n` +
                   `Booking Reference: ${referenceCode}\n\n`;
            
            // Show previous appointment
            if (oldAppointmentDate || oldAppointmentTime) {
                body += `Previous Appointment:\n`;
                if (oldAppointmentDate) {
                    body += `Date: ${formatDateForGmail(oldAppointmentDate)}\n`;
                }
                if (oldAppointmentTime) {
                    body += `Time: ${oldAppointmentTime}\n`;
                }
                body += `\n`;
            }
            ...
            if (newAppointmentDate) {
                body += `Date: ${formatDateForGmail(newAppointmentDate)}\n`;
            }
            if (newAppointmentTime) {
                body += `Time: ${newAppointmentTime}\n`;
            }
            
            // Add reschedule message if provided
            if (rescheduleMessage.trim()) {
                body += `\n${rescheduleMessage.trim()}\n`;
            } else {
                // Default reschedule message
                body += `\nWe have rescheduled your appointment. Please see the new appointment details above. If you have any questions or concerns, please don't hesitate to contact us.\n`;
            }
        } else {
            // Fallback: use current status if no changes
            emailType = 'Status Update';
    ...
    // Log Gmail compose action to audit log
    const formData = new FormData();
    formData.append('log_gmail_compose', '1');
    ...
    // Send log request (fire and forget - don't wait for response)
    fetch(window.location.href, {
        method: 'POST',
        body: formData
    }).catch(err => {
        console.error('Failed to log Gmail compose:', err);
    });
